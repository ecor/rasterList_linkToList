...
The 'rasterList' package has been A S4 class has been created such that complex
operations can be     executed on each cells of a raster map. The raster of objects
contains the     traditional raster map with the addition of a list of generic
objects: one     object for each raster cells. It allows to write few lines of R
code for complex     map algebra. Two environmental applications about frequency
analysis of raster     map of precipitation and creation of a raster map of soil
water retention curves     have been presented. built to perform a PSO Calibration
through [hydroPSO](https://cran.r-project.org/package=hydroPSO)  package of the
the results of the physically-based hydrological model GEOtop. The package makes
use of the GEOtop-R plugin
[geotopbricks](https://cran.r-project.org/package=geotopbricks), which
import/export GEOtop Data into R. 
...
library(Extremes)
hwmi_ <- function(x,ref=x,format_date="X%Y.%m.%d",...) {
	## Heat Magnitude Heat Wave Index
	
	yTemp_ <- as.numeric(as.character(as.Date(names(x),format=format_date),format="%Y"))
	
	yTemp <- yTemp_[1]
	Temp <- x
	
	yTref <- as.numeric(as.character(as.Date(names(ref),format=format_date),format="%Y"))
	
	
	print(yTref)
	Tref <- ref[yTref %in% (yTref[1]+0:31)] ## NOT MORE THEN #@ YEARS FOR REFERENCE PERIOD!!!
	### IT WORKS WITH 30 32 YEARS
	
	
	
	hout <- hwmi(yTref=yTref[1],Tref=Tref,yTemp=yTemp,Temp=Temp)
	out_hwmid <- hwmid(yTref=yTref[1],Tref=Tref,yTemp=yTemp,Temp=Temp)
	##### Readible format: 
	##### http://onlinelibrary.wiley.com/doi/10.1002/2014JD022098/full
	out <- as.data.frame(hout$hwmi)
	names(out) <- c("hwmi","duration","doy")
	out$year <- yTemp+1:nrow(out)-1
	attr(out,"thr") <- hout$thr
	attr(out,"pdfx") <- hout$pdfx
	attr(out,"pdfy") <- hout$pdfy
	attr(out,"hwmid") <- out_hwmid
	######
	
	
	### hwmi produce an arror if Tref and Temp cover the same period!!!
	###
	###
...
See the following functionalieties with Carcassonne ....
